...
namespace Drawing
{
	...
	public:
		...
		Drawing::QuadChanPixel<TPIXEL> convertPixel(const TUNITINTERVAL& value)
		{
			...
			if (value == 1.0)
				return Drawing::QuadChanPixel<TPIXEL>(_colorMap->Values[0][size], _colorMap->Values[1][size], _colorMap->Values[2][size]);

			TUNITINTERVAL spacing = ((TUNITINTERVAL)1 / (size - 1));
			UInt8 index = (UInt8)(value / spacing);

			return Drawing::QuadChanPixel<TPIXEL>(value, 14, 21);
		}
	};
